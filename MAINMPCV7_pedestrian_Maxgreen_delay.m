function [optimal_green_times, optimal_alpha_enter, ...
          ...
          MAINMPCV7_pedestrian_Maxgreen_delay(params, current_state, timestep, prediction_horizon, ...
                  control_horizon, cycle_times, ...
                   min_green_times, pedestriancrossing_Maxgreen)
    ...
    %% 4) Green Time Bounds & Cycle Constraints
       % Define the mapping: each row corresponds to an intersection and its associated links.
    ...
    for k = 1:H
        for i = 1:num_links
            for d = 1:num_directions
                constraints = [constraints, ...
                    min_green_times(i,d) <= green_times(i,d,k) <= pedestriancrossing_Maxgreen(i,d,k)];
            end
        end
        ...
        for inter = 1:size(intersection_links,1)
            ...
            constraints = [constraints, ...
                sum(sum(green_times(L,:,k))) == cycle_times(inter)];
        end
    end
