function Data = runModelPedMaxGreen(params)
% runModelPedMaxGreen runs the simulation with pedestrian delays using max-green times.
...
%   n_log, q_log, green_times_log, and alpha_leave_log
    ...
    %max_green_times = params.max_green_times; not needed because maxgreen
    %is via pedestriancrossing
    min_green_times = params.min_green_times; % [8 x 3]
    pedestriancrossing_Maxgreen = params.pedestriancrossing_Maxgreen;
   ...
   if model_choice == 2
    ...
    %% Preallocate logs (as in Script 2.1)
    ...
    %% Simulation loop (Scripts 2.1–2.3)
    for timestep = 1:total_timesteps
        disp(['Model PedMaxGreen - Timestep: ', num2str(timestep)]);
        
        % Solve the MPC with pedestrian max-green delay (Script 2.2)
        [optimal_green_times, optimal_alpha_enter, optimal_alpha_arrive, optimal_alpha_leave, alpha_leave_update] = ...
            MAINMPCV7_pedestrian_Maxgreen_delay(params, current_state, timestep, prediction_horizon, control_horizon, cycle_times, min_green_times, pedestriancrossing_Maxgreen);
        ...
        % Apply the state update (Script 2.3)
        current_state = MAINCALCULATEQUESV7_pedestrian_Maxgreen_delay(current_state, alpha_enter_log(:,timestep), alpha_arrive_log(:,:,timestep), alpha_leave_log(:,:,timestep));
    ...
    %% --- Additional Plots ---
    % Define time vector for the simulation actions
    t = 1:total_timesteps;
    ...
    title('Combined Leaving Flows (Model PedMaxGreen)');
    legend(arrayfun(@(x) sprintf('Node %d', x), 1:8, 'UniformOutput', false), 'Location','Best');
    grid on;
    print(fig_leave, 'PedMaxGreen_CombinedLeavingFlows.png', '-dpng', '-r300');
    ...
    title('Combined Green Times (Model PedMaxGreen)');
    legend(arrayfun(@(x) sprintf('Node %d', x), 1:8, 'UniformOutput', false), 'Location','Best');
    grid on;
    print(fig_green, 'PedMaxGreen_CombinedGreenTimes.png', '-dpng', '-r300');
    ...
    title('Entering Flows (Model PedMaxGreen)');
    legend(arrayfun(@(x) sprintf('Node %d', x), 1:8, 'UniformOutput', false), 'Location','Best');
    grid on;
    print(fig_enter, 'PedMaxGreen_EnteringFlows.png', '-dpng', '-r300');
    ...
    title('Combined Arriving Flows (Model PedMaxGreen)');
    legend(arrayfun(@(x) sprintf('Node %d', x), 1:8, 'UniformOutput', false), 'Location','Best');
    grid on;
    print(fig_arrive, 'PedMaxGreen_CombinedArrivingFlows.png', '-dpng', '-r300');
    ...
    title('Combined Queue Lengths (Model PedMaxGreen)');
    legend(arrayfun(@(x) sprintf('Node %d', x), 1:8, 'UniformOutput', false), 'Location','Best');
    grid on;
    print(fig_queue, 'PedMaxGreen_CombinedQueueLengths.png', '-dpng', '-r300');
